...
async fn main() {
    ...
    // Step 1: LocalSet-wrapped binding
    let (sink, stream, addr) = LocalSet::new().run_until(async move {
        dfir_rs::util::bind_tcp_lines(addr).await
    }).await;
    
    // Step 2: DFIR graph definition (outside LocalSet)
    let mut flow = dfir_syntax! {
        source_stream(stream) -> dest_sink(sink);
    };
    
    // Step 3: LocalSet-wrapped execution
...
- [TCP Echo Server](https://github.com/hydro-project/hydro/blob/main/dfir_rs/examples/echo_server_tcp/main.rs)
