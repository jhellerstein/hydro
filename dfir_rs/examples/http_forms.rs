...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("  Content-Type: {}", login_request.headers.get("Content-Type").unwrap());
    ...
    update_data.insert("bio".to_string(), "Software Engineer & Coffee Enthusiast".to_string());
    ...
    println!("  Content-Type: {}", update_request.headers.get("Content-Type").unwrap());
    ...
    println!("  Has email field: {}", form_request.has_form_field("email"));
    println!("  Has phone field: {}", form_request.has_form_field("phone"));
    ...
    println!("\n🔧 Manual form parsing:");
    let body = "action=subscribe&topic=newsletter&frequency=weekly&format=html".as_bytes().to_vec();
    let mut manual_request = HttpRequest::post("/newsletter", body);
    manual_request.headers.insert("Content-Type".to_string(), "application/x-www-form-urlencoded".to_string());
    
    println!("  Before parsing: {} form fields", manual_request.form_data.len());
    manual_request.parse_form_data();
    println!("  After parsing: {} form fields", manual_request.form_data.len());
    ...
    let special_chars_form = HttpRequest::post_form("/feedback", {
        let mut data = HashMap::new();
        data.insert("comment".to_string(), "This has spaces & special chars!".to_string());
    ...
    println!("  Encoded body: {}", String::from_utf8_lossy(&special_chars_form.body));
